"""Legacy implementation retained for rollback/reference; not registered in the live metric registry."""
...
from poker.metrics.base import Metric
...
def extract_hero_call_spots(hand: Hand) -> list[CallSpot]:
    ...
    for act in hand.actions:
        ...
        facing = aggression
        aggression = None
        if facing is None or facing.pot_before <= 0 or "Hero" not in active or facing.player not in active:
            continue
...
def _matching_call_spots(hand: Hand, options: dict[str, Any]) -> list[CallSpot]:
    ...
    return [spot for spot in extract_hero_call_spots(hand)
...
class LegacyWhenICallMetric(Metric):
    id = "when_i_call_legacy"
    name = "When I Call (Legacy)"
    description = "旧版 Hero call 后按 Facing Bet / Facing Raise 统计的参考实现"
    chart_type = "stats"
    ...
    def compute(self, dataset: HandDataset, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if heads_up_grid:
            grid = {
                ...
                "cells": [
                    {"hand": label, "count": revealed_counts.get(label, 0),
        ...
        return {
            ...
            "options": {
                ...
                "sizes": wir._as_str_list(opts.get("sizes")) or list(wir.BET_SIZE_IDS),
